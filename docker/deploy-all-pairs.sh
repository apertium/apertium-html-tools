...
docker-compose exec apy bash -c "apt-get update && apt-get upgrade -y"
docker-compose exec apy bash -c "cd /source && ./apertium-get -m | grep -v '^$\|^\s*\#' | sort | xargs -n 1 -P 6 timeout 10m ./apertium-get -s"
