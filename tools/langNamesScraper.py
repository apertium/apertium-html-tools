...
def getApertiumLanguages():
    dirs = [('incubator', r'apertium-(\w{2,3})(?:-(\w{2,3}))?'), 
            ('nursery', r'apertium-(\w{2,3})(?:-(\w{2,3}))?'), 
            ('staging', r'apertium-(\w{2,3})(?:-(\w{2,3}))?'), 
            ('trunk', r'apertium-(\w{2,3})-(?:\w{2,3})'), 
            ('languages', r'apertium-(\w{3})'), 
           ]
    for (dirPath, dirRegex) in dirs:
        svn_data = str(subprocess.check_output('svn list --xml https://svn.code.sf.net/p/apertium/svn/%s/' % dirPath, stderr=subprocess.STDOUT, shell=True), 'utf-8')
        for langCodes in re.findall(svn_data, dirRegex, re.DOTALL):
            apertiumLanguages.update(langCodes)
    ...
    for locale in args.languages:
        ...
        try:
            ...
            for language in languages:
                if language.text:
                    if not args.apertiumOnly or (args.apertiumOnly and locale[1] in apertiumLanguages):
